...
function mostraServiziAJAX_new(categorie, selezione, coordinateSelezione, selectOption){
    ...
    if (coordinateSelezione != "") {
        if (undefined != coordinateSelezione) {
    ...
    function mostraComune(selectOption) {
        ...
        if (selectOption.options.selectedIndex != 0){
            var categorie = [];
            $('#categorie').find(':checked').each(function() {
                categorie.push($(this).val());
            });
            var stringaCategorie = categorie.join(";");
            if (stringaCategorie == ""){
    ...
    /** function retrieve the specific param from a url String */
    ...
    /**  abilitate the funxtion getUrkls on a psecific eleemtn HTML */
    $('.edit').click(function() {
        var idVar = "id";
        var href = $(this).attr("href");
        var test = getUrlVars(href)[idVar];
        alert(test);
    });
